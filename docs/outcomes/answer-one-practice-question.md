...
**If the parent yanks the phone away before Ruairidh has tapped any button:** no answer is registered. When he returns to the app later, the same Practice question is re-presented from the beginning. The Practice Session outcome (separate, not yet written) is responsible for persisting which question was in flight.

**If the parent yanks the phone away after a wrong tap but before a correct one:** the same question reloads on session resume, with his prior wrong taps still visibly red. He continues from exactly where he left off. (The Practice Session outcome is responsible for snapshotting which buttons were tapped wrong, in what order.)
...
12. After three consecutive failed attempts to play the question audio, the screen briefly shows "Trying another one" and a different question on the same module loads in its place.
...
15. If the phone is taken away and Ruairidh returns to the app later, this specific question is preserved with his prior wrong-tap state intact (red buttons remain red). Verifying this is the responsibility of the Practice Session outcome; listed here for cross-reference.

**Notes for the QA agent:**
- Items 1–10 are testable in a browser using Playwright (load module, simulate taps, inspect button states, assert audio playback).
...
- Item 15 is verified by the Practice Session outcome, not this one.
...
- The signal carries the timestamp of completion and identifies which module the question came from.
...
- No persisted in-flight question state (the wrapping Practice Session outcome handles snapshotting current question + wrong-tap state on session interruption, not this outcome).

---

## Field 6: Dependencies

- **Active Practice session on a specific module** — the wrapping Practice Session outcome (separate, not yet written) must have started and not yet ended. This outcome cannot fire from a cold start.
- **The module's question bank is loaded** — audio files for each question, button labels for each answer option, and the mapping of which buttons are correct for each question. This is content / configuration provided by the module-loading outcome (also separate).
...
- **The current question has been selected and queued for presentation** by the wrapping Practice Session outcome.

This outcome can be built once the Practice Session outcome and the Module Loading outcome are both at least drafted, but it cannot be verified end-to-end until those wrapping outcomes are also implemented.

---

## Forward notes for the next outcome

Logged during this outcome's writing — relevant when we write the Test-question outcome and the Practice Session outcome.
...
**Same-question-on-resume rule (for the Practice Session outcome):**
...
- The Practice Session outcome is the one responsible for this persistence — this outcome only consumes the queued question and produces the completion signal.
